...
// =====================================================
// CINEMATIC HEADLINE FX — Auto word wrapping
...
(function () {
  ...
  function wrapTextNodeWords(textNode, delayState) {
    ...
    parts.forEach((part) => {
      ...
      span.className = 'fx-headline-word';
      span.dataset.word = part;
      span.textContent = part;
      span.style.animationDelay = `${delayState.index * 0.075}s, ${delayState.index * 0.075}s, ${delayState.index * 0.075}s`;
      delayState.index += 1;
  ...
  function enhanceHeadline(el) {
    if (!el || el.dataset.fxHeadline === 'true') return;
    el.dataset.fxHeadline = 'true';
    ...
    const walker = document.createTreeWalker(
      ...
      NodeFilter.SHOW_TEXT,
      {
        acceptNode(node) {
          ...
          if (parent.classList.contains('fx-headline-word')) return NodeFilter.FILTER_REJECT;
    ...
    nodes.forEach((node) => wrapTextNodeWords(node, delayState));

    requestAnimationFrame(() => {
      el.classList.add('fx-headline-ready');
    });
  }

  function initCinematicHeadlineFx() {
    const selectors = [
      '.hero-v7 h1',
      '.mega-title',
      '.project-hero h1',
    ...
    document.querySelectorAll(selectors.join(',')).forEach(enhanceHeadline);
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initCinematicHeadlineFx);
  } else {
    initCinematicHeadlineFx();
  }
})();